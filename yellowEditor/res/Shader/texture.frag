...
vec3 calcDirLight(DirLight light)
{
	...
	vec3 ambient = light.ambient.rgb * texture(u_Material.diffuse, v_TexCoord).rgb;
	vec3 diffuse = light.diffuse.rgb * diff * texture(u_Material.diffuse, v_TexCoord).rgb;
	vec3 specular = light.specular.rgb * spec * texture(u_Material.specular, v_TexCoord).rgb;
