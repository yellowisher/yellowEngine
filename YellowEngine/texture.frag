...
void main()
{
	...
	float distanceFromLight = length(u_Light.position - v_FragPosition);
	float attenuation = 1.0 / (
			u_Light.constant + 
			u_Light.linear * distanceFromLight + 
			u_Light.quadratic * distanceFromLight * distanceFromLight);
    o_FragColor = vec4((ambient + diffuse + specular) * attenuation, 1.0f);
}